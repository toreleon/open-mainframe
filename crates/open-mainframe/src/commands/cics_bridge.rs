...
//!
//! Connects the COBOL interpreter to the open-mainframe-cics runtime, dispatching
//! EXEC CICS commands (SEND MAP, RECEIVE MAP, RETURN, READ, etc.)
//! to the appropriate runtime methods.

use std::collections::HashMap;

use open_mainframe_cics::bms::BmsMap;
use open_mainframe_cics::runtime::{CicsRuntime, FileMode, FileRecord};
use open_mainframe_cics::terminal::{TerminalCallback, TerminalManager};
...
/// Concrete CICS command handler that dispatches to open-mainframe-cics runtime.
...
impl CicsBridge {
    ...
    pub fn register_file(
        ...
        let file = open_mainframe_cics::runtime::CicsFile::new(name, record_length, key_length);
    ...
    fn handle_send_map(
        ...
        let cursor_pos = Self::get_option_str(options, "CURSOR")
            ...
            .map(|offset| {
                ...
                open_mainframe_cics::terminal::ScreenPosition::new(row, col)
            });
        ...
        if let Some(ref mut cb) = self.terminal_callback {
            ...
            let placeholder = open_mainframe_cics::bms::BmsMap::default();
    ...
    fn handle_receive_map(
        ...
        self.runtime.eib.set_aid(open_mainframe_cics::runtime::eib::aid::ENTER);
        self.runtime.eib.set_response(CicsResponse::Normal);
        Ok(())
    }
...
mod tests {
    ...
    use open_mainframe_tui::mock::MockTerminal;
    ...
    fn test_handle_aid_command() {
        ...
        assert_eq!(
            bridge.get_aid_handler_for_byte(open_mainframe_cics::runtime::eib::aid::PF3),
            Some("END-PARA")
        );
        assert_eq!(
            bridge.get_aid_handler_for_byte(open_mainframe_cics::runtime::eib::aid::ENTER),
            Some("PROCESS-PARA")
        );
        // No handler for PF1
        assert_eq!(
            bridge.get_aid_handler_for_byte(open_mainframe_cics::runtime::eib::aid::PF1),
            None
        );
    }
    ...
    fn test_receive_map_sets_field_variables() {
        ...
        mock.queue_input(open_mainframe_cics::runtime::eib::aid::ENTER, fields);
        bridge.set_terminal_callback(Box::new(mock));
        ...
        // Verify AID was set
        assert_eq!(bridge.runtime.eib.eibaid, open_mainframe_cics::runtime::eib::aid::ENTER);
    }
}
