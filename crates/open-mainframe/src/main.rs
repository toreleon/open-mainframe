...
/// Find a COBOL source file for a program name in a directory.
fn find_program_source(program_name: &str, search_dirs: &[PathBuf]) -> Option<PathBuf> {
    ...
    for dir in search_dirs {
        ...
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                if let Some(stem) = name_str.strip_suffix(".cbl")
...
// ---------------------------------------------------------------------------
// Shared helper: set up EIB fields in the interpreter environment
// ---------------------------------------------------------------------------
...
fn setup_env(
...

    {
        ...
        env.set("EIBTRNID", CobolValue::alphanumeric(
            eib.transaction_id(),
        )).ok();
    ...
    if let Some(ref ca) = commarea {
        env.set(
            "EIBCALEN",
            CobolValue::from_i64(ca.len() as i64),
        ).ok();
        let ca_str = String::from_utf8_lossy(ca).to_string();
        env.set(
            "DFHCOMMAREA",
            CobolValue::alphanumeric(ca_str),
        ).ok();
    }
